...
- 🟢 **从 Chrome 网上应用店安装（推荐 · 免构建）**：打开 👉 **[Chrome 网上应用店页面](https://chromewebstore.google.com/detail/eplcnheinfmkcckelinolhpdagbamdcc)** → 点「**添加至 Chrome**」。装好后点工具栏的扩展图标（可右键固定）即可。**无需克隆代码、无需构建、无需自建飞书应用。**
...
> ℹ️ **商店版授权提示**：商店版的飞书应用由发布者预置。若点「飞书授权」时提示**「应用不在可用范围 / 无权限」**，说明该应用仅对特定企业开放——这时请改用上面的「**自行构建**」方案，用**你自己的**飞书应用授权（见 [`QUICKSTART.md`](QUICKSTART.md)）。大模型 Key 始终由你自己在「设置」里填、只存本机。
